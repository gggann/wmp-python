...
def on_confirm_click():
    ...
    try:
        ...
        tk.messagebox.showwarning(title='提示', message='请关闭Excel文件后再执行程序！')
        return
...
def get_input_value():
    ...
    if not input_str.isdigit():
        tk.messagebox.showwarning(title='提示', message='请输入整数！')
        return None
    input_int = int(input_str)
    if input_int >= 91:
        tk.messagebox.showwarning(title='提示', message='请输入小于91的数！')
        return None
    print(input_int)
    return input_int
...
input_label.grid(row=1, column=0, padx=(0, 87), pady=10, sticky=tk.E)
...
# 创建 Checkbox 变量和控件，并将其与 checkbox_changed 函数关联
ctlg_var = tk.IntVar()
ctlg_checkbox = tk.Checkbutton(root, text='ctlg', variable=ctlg_var, command=checkbox_changed)
ctlg_checkbox.grid(row=1, column=0, padx=(100, 10), pady=10, sticky=tk.W)
